...
![Build Badge](https://github.com/ksktechai/LiveStock-AI/actions/workflows/ci.yml/badge.svg)
...
_Image generated by Nano Banana_ 

## Overview

![General Overview](docs/general-overview.png)
...
- **AI Sentiment Analysis**: Powered by **Ollama (Llama 3.2)** for local, private analysis.
- **Backend**: Built with **Spring Boot 4.0.0** and Spring WebFlux for reactive/async processing.
...
_LiveStock AI uses a local LLM via Ollama by default. OpenAI integration is optional and pluggable, but not required to run the system._


---

## Prerequisites
- **Java 25** (Required for Spring Boot 4.0.0 features)
- **Node.js** (for Frontend)
- **NewsAPI Key** (Get one free at [newsapi.org](https://newsapi.org))
- **Ollama** (Optional, for local AI analysis), optional if using Mock profile)
